...
SpoofedConvar::SpoofedConvar(const char* szCVar) {
	m_pOriginalCVar = game_interfaces::CVar->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::~SpoofedConvar() {
	if (IsSpoofed()) {
		...
		game_interfaces::CVar->UnregisterConCommand(m_pDummyCVar);
		free(m_pDummyCVar);
...
void SpoofedConvar::Spoof() {
	if (!IsSpoofed() && m_pOriginalCVar) {
		...
		m_pDummyCVar = new ConVar();
		memcpy(m_pDummyCVar, m_pOriginalCVar, sizeof(ConVar));

		m_pDummyCVar->pNext = nullptr;
		//Register it
		game_interfaces::CVar->RegisterConCommand(m_pDummyCVar);
