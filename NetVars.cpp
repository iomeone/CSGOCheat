...
void NetvarSys::Initialize()
{
	...
	for (auto clientclass = game_interfaces::Client->GetClasses();
		clientclass != nullptr;
	...
	Logger::SaveToLog("Netvars loaded");
}

netvar_table NetvarSys::LoadTable(RecvTable* recvTable)
{
...
uint32_t NetvarSys::GetOffset(const netvar_table& table, const std::string& propName)
{
	for (const auto& prop : table.child_props) {
...
RecvProp* NetvarSys::GetNetvarProp(const netvar_table& table, const std::string& propName)
{
	for (const auto& prop : table.child_props) {
		if (strncmp(prop->m_pVarName, propName.data(), propName.size()) == 0) {
			return prop;
...
NetvarSys* netvars = new NetvarSys();